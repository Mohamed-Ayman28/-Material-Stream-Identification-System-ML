...
def train_svm_grid(X_train, y_train, cv=3, n_jobs=-1, verbose=1, fast_mode=False):
    """
    ...
    Args:
        fast_mode: If True, uses a smaller parameter grid for faster training
    """
    svc = SVC(probability=True, random_state=RANDOM_STATE, cache_size=500)
    
    if fast_mode:
        # Reduced grid for faster training (6 combinations instead of 16)
        param_grid = {
            'kernel': ['rbf', 'linear'],
            'C': [1, 10],
            'gamma': ['scale']
